...
export default function WealthTab({ token, preferredMarket }) {
  ...
  useEffect(() => {
    async function load() {
      setLoading(true)
      try {
        const [sugRes, tradesRes, wfoRes] = await Promise.all([
  ...
  const evalStats = useMemo(() => {
    const evaled = suggestions.filter(s => s.status !== 'PENDING' && s.status !== 'EVALUATED')
    const hits = evaled.filter(s => s.status === 'HIT')
    const byTier = { '5pct': { total: 0, hits: 0 }, '3pct': { total: 0, hits: 0 } }
    evaled.forEach(s => {
      const t = s.tier
      if (byTier[t]) { byTier[t].total++; if (s.status === 'HIT') byTier[t].hits++ }
    })
    return {
      totalEvaluated: evaled.length,
      overallHitRate: evaled.length ? (hits.length / evaled.length * 100).toFixed(0) : '-',
      byTier,
    }
  }, [suggestions])
        ...
        <div style={{ background: '#1e293b', borderRadius: 12, border: '1px solid #334155', padding: 20 }}>
          <div style={{ display: 'flex', gap: 24, flexWrap: 'wrap', marginBottom: 16 }}>
            <div>
              ...
              <div style={{ color: '#64748b', fontSize: 11 }}>{evalStats.totalEvaluated} evaluated</div>
            </div>
            <div style={{ borderLeft: '1px solid #334155', paddingLeft: 24 }}>
              <div style={{ color: '#94a3b8', fontSize: 12 }}>🎯 5% Tier</div>
              <div style={{ color: '#a3e635', fontSize: 20, fontWeight: 700 }}>
                {evalStats.byTier['5pct'].total ? (evalStats.byTier['5pct'].hits / evalStats.byTier['5pct'].total * 100).toFixed(0) : '-'}%
              </div>
              <div style={{ color: '#64748b', fontSize: 11 }}>
                {evalStats.byTier['5pct'].hits}/{evalStats.byTier['5pct'].total} trades
              </div>
            </div>
            <div style={{ borderLeft: '1px solid #334155', paddingLeft: 24 }}>
              <div style={{ color: '#94a3b8', fontSize: 12 }}>📈 3% Tier</div>
              <div style={{ color: '#60a5fa', fontSize: 20, fontWeight: 700 }}>
                {evalStats.byTier['3pct'].total ? (evalStats.byTier['3pct'].hits / evalStats.byTier['3pct'].total * 100).toFixed(0) : '-'}%
              </div>
              <div style={{ color: '#64748b', fontSize: 11 }}>
                {evalStats.byTier['3pct'].hits}/{evalStats.byTier['3pct'].total} trades
              </div>
            </div>
          </div>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 8 }}>
            {[14, 30, 63, 90].map(d => {
              const rows = suggestions.filter(s => s.evaluated && s[`actual_return_${d}d`] != null)
              const hits = rows.filter(s => {
                const v = s[`actual_return_${d}d`]
                const target = s.tier === '5pct' ? 5 : s.tier === '3pct' ? 3 : null
        ...
        <div style={{ marginBottom: 24 }}>
          <div style={{ background: '#1e293b', borderRadius: 12, border: '1px solid #334155', padding: 16 }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 16, flexWrap: 'wrap' }}>
              <span style={{ color: '#94a3b8', fontSize: 13 }}>Walk-Forward OOS Validation:</span>
              {wfo.gate_status ? (
                <span style={{
                  background: wfo.gate_status === 'GREEN' ? '#14532d' : wfo.gate_status === 'AMBER' ? '#78350f' : '#7f1d1d',
                  color: wfo.gate_status === 'GREEN' ? '#4ade80' : wfo.gate_status === 'AMBER' ? '#fbbf24' : '#f87171',
                  padding: '3px 12px', borderRadius: 12, fontSize: 13, fontWeight: 600,
                }}>{wfo.gate_status}</span>
              ) : (
                <span style={{ color: '#fbbf24', fontSize: 13 }}>
                  Accumulating data — {wfo.days_accumulated || '?'} of 30 days
                </span>
              )}
              {wfo.h30?.sharpe != null && (
                <span style={{ color: '#94a3b8', fontSize: 12 }}>
                  30d Sharpe: {wfo.h30.sharpe?.toFixed(2)} | 63d: {wfo.h63?.sharpe?.toFixed(2) || '-'} | 90d: {wfo.h90?.sharpe?.toFixed(2) || '-'}
                </span>
              )}
            </div>
          </div>
                  ...
                  suggestions.slice(0, 100).map((s, i) => (
                    <tr key={i} style={{ borderBottom: '1px solid #1e293b' }}>
                      ...
                      <td style={{ padding: '8px 12px' }}>
                        <span style={{
                          background: s.tier === '5pct' ? '#365314' : s.tier === '3pct' ? '#1e3a5f' : '#1e293b',
                          color: s.tier === '5pct' ? '#a3e635' : s.tier === '3pct' ? '#60a5fa' : '#64748b',
                          padding: '1px 8px', borderRadius: 8, fontSize: 11, fontWeight: 600,
                        }}>{s.tier === '5pct' ? '5%' : s.tier === '3pct' ? '3%' : s.tier || '-'}</span>
                      </td>
